...
def handle_keys(game_object):
    ...
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            pygame.quit()
            raise SystemExit
        if event.type == pygame.KEYDOWN and event.key in HANDLE_KEYS_DIRECTIONS:
            new_direction, opposite_direction = HANDLE_KEYS_DIRECTIONS[event.key]
...
class Apple(GameObject):
    ...
    def randomize_position(self, occupied_positions):
        ...
        Args:
            occupied_positions (list of tuples): A list of (x, y) positions that are
                currently occupied (e.g., by the snake's body).

        Raises:
            Exception: If there are no free cells left on the board (i.e., when
                the number of occupied positions equals or exceeds the total number
                of grid cells).
        """
